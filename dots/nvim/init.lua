	...
	items = {
		{
			...
			{ name = "config  󰒓", action = "edit /etc/nixos/dots/nvim/init.lua", section = "Actions" },
			{ name = "explorer  󰙅", action = "NvimTreeOpen", section = "Actions" },
...
-- Mini starter highlight groups (stylix palette, via vim.g from modules/nvim/_parts/lua.nix)
local c = vim.g.stylix_colors
vim.api.nvim_set_hl(0, "MiniStarterHeader", { fg = c.base0A, bold = true })
...
vim.api.nvim_set_hl(0, "MiniStarterCurrentBullet", { fg = c.base0A, bold = true })
	...
	callback = function(ev)
		...
		vim.api.nvim_create_autocmd("BufWinEnter", {
			...
			callback = function()
				...
				if ok then
					_ibl.setup_buffer(0, { enabled = false })
				end
			end,
		})
	end,
